...
public class AlarmCallBack implements HCNetSDK.FMSGCallBack {
    private AlarmCallbackHandler alarmCallbackHandler;

    public AlarmCallBack(AlarmCallbackHandler alarmCallbackHandler) {
        ...
        switch (lCommand) {
            case HCNetSDK.COMM_ALARM_V40:
                ...
                switch (struAlarmInfoV40.struAlarmFixedHeader.dwAlarmType) {
                    case 0:
                        ...
                        alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, msg));
                        break;
                    case 1:
                        msg = msg + "：硬盘满";
                        alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, msg));
                ...
                try {
                    for (int i = 0; i < strPlateResult.struPlateInfo.sLicense.length; i++) {
                        if (strPlateResult.struPlateInfo.sLicense[i] == 0) {
                            ...
                            alarmCallbackHandler.handle(new AlarmResult(AlarmType.ANY_CAR, msg, plateNumber, HCSDK.getNameByid(pAlarmer.lUserID)));
                            break;
                        }
                    }
            ...
            case HCNetSDK.COMM_ITS_PLATE_RESULT:
                ...
                try {
                    ...
                    alarmCallbackHandler.handle(new AlarmResult(AlarmType.NEW_ANY_CAT, msg, srt3, HCSDK.getNameByid(pAlarmer.lUserID)));
                } catch (UnsupportedEncodingException e1) {
                    // TODO Auto-generated catch block
                    e1.printStackTrace();
                } catch (IOException e) {
            ...
            case HCNetSDK.COMM_ALARM_PDC:
                //客流量统计
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "客流量统计"));
                break;

            case HCNetSDK.COMM_ITS_PARK_VEHICLE:
                //停车场数据
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "停车场数据"));
                break;
            case HCNetSDK.COMM_ALARM_TFS:
                //违章拍照
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "违章拍照"));
                break;
            case HCNetSDK.COMM_ALARM_AID_V41:
                //交通事件报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "交通事件报警"));
                break;
            case HCNetSDK.COMM_ALARM_TPS_V41:
                //交通统计信息报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "交通统计信息报警"));
                break;
            case HCNetSDK.COMM_UPLOAD_FACESNAP_RESULT:
                //人脸识别
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "人脸识别"));
                break;
            case HCNetSDK.COMM_SNAP_MATCH_ALARM:
                //人脸黑名单识别
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "人脸黑名单识别"));
                break;
            case HCNetSDK.COMM_ALARM_ACS: //门禁主机报警信息
                //门禁报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "门禁报警"));
                break;
            case HCNetSDK.COMM_ID_INFO_ALARM: //身份证信息
                //身份证刷卡报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "身份证刷卡报警"));
                break;
            case HCNetSDK.COMM_ISAPI_ALARM: //ISAPI协议报警信息
                //ISAPI协议报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, "ISAPI协议报警"));
                break;
                    ...
                    for (int i = 0; i < whiteCarResult.sLicense.length; i++) {
                        if (whiteCarResult.sLicense[i] == 0) {
                            ...
                            if(whiteCarResult.byListType == 0){
                                ...
                                alarmCallbackHandler.handle(new AlarmResult(AlarmType.WHITE_LIST, msg, plateNumber, HCSDK.getNameByid(pAlarmer.lUserID)));
                            }else if(whiteCarResult.byListType == 1){
                                msg = msg + "：检测到黑名单车辆，车牌：" + plateNumber;
                                alarmCallbackHandler.handle(new AlarmResult(AlarmType.BLACK_LIST, msg, plateNumber, HCSDK.getNameByid(pAlarmer.lUserID)));
                            }else if (whiteCarResult.byListType == 2){
                                msg = msg + "：检测到临时车辆，车牌：" + plateNumber;
                                alarmCallbackHandler.handle(new AlarmResult(AlarmType.TEMP_LIST, msg, plateNumber, HCSDK.getNameByid(pAlarmer.lUserID)));
                            }
            ...
            default:
                //其他报警
                alarmCallbackHandler.handle(new AlarmResult(AlarmType.OTHER, msg, "", HCSDK.getNameBySSerialNumber(pAlarmer.sSerialNumber)));
                break;
        }
    }
}
