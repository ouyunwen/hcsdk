...
public class BarrierSDK extends HCSDK {
     ...
     *
     * @param deviceName         道闸的设代号（在{@link HCSDK#connect(String, int, String, String, String)}中声明）
    ...
    public static void addPass(String deviceName, String licensePlateNumber, LocalDateTime startTime, LocalDateTime endTime) throws InstructionExecuteException {
        ...
        if (lHandle > -1) {
            ...
            synchronized (lock) {
                ...
                if (!bSend) {
                    throw new InstructionExecuteException("调用NET_DVR_SendRemoteConfig时异常，错误码：" + hcNetSDK.NET_DVR_GetLastError());
                }
            }
        } else {
            throw new InstructionExecuteException("调用NET_DVR_StartRemoteConfig方法时异常。");
        }
    }
     ...
     * @return 连接id
     */
    private static int openConnection(String deviceName) throws InstructionExecuteException {
        //根据名称获取存留在sdk的设备id
        ...
        //发起远程连接
        return hcNetSDK.NET_DVR_StartRemoteConfig(id, HCNetSDK.NET_DVR_VEHICLELIST_CTRL_START, null, 0, fVehicleCrtlCB, Pointer.NULL);
    }
